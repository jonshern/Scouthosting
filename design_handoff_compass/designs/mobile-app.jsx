  ...
  return (
    <IOSDevice width={402} height={874} title="Compass · Event">
      <div style={{ background: p.bg, minHeight: '100%', paddingBottom: 120, fontFamily: T.ui }}>
        ...
        {/* Weather forecast — appears as soon as we're inside 10-day window */}
        <div style={{ padding: '20px', borderBottom: `1px solid ${p.line}` }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontSize: 11, color: p.inkMuted, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600 }}>Weather · Birch Lake</div>
            <div style={{ fontSize: 10, color: p.inkMuted, fontStyle: 'italic' }}>Updated 2:14 PM · NWS</div>
          </div>
          {/* 3-day strip — weather available as soon as forecast is */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 10 }}>
            {[
              { d: 'Fri', label: 'Departure', hi: 58, lo: 41, cond: 'Partly cloudy', icon: 'cloud-sun', alert: false },
              { d: 'Sat', label: 'Camp day', hi: 52, lo: 36, cond: 'Showers likely', icon: 'rain', alert: true, pop: 70 },
              { d: 'Sun', label: 'Return', hi: 61, lo: 39, cond: 'Sunny', icon: 'sun', alert: false },
            ].map((w, i) => {
              const Icon = () => {
                if (w.icon === 'sun') return (
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="12" cy="12" r="4" fill="#fbbf24" stroke="#f59e0b"/>
                    <path d="M12 3v2M12 19v2M3 12h2M19 12h2M5.6 5.6l1.4 1.4M17 17l1.4 1.4M5.6 18.4 7 17M17 7l1.4-1.4"/>
                  </svg>
                );
                if (w.icon === 'rain') return (
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={p.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M16 13a4 4 0 0 0-8 0c-2.5 0-4 1.5-4 4 0 1 .5 2 1 2.5h13a4 4 0 0 0 0-8 4 4 0 0 0-2 .5" fill="#cbd5e1" stroke={p.inkMuted}/>
                    <path d="M8 19v3M12 19v3M16 19v3" stroke={p.accent} strokeWidth="2.5"/>
                  </svg>
                );
                return (
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={p.inkMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="8" cy="9" r="3" fill="#fbbf24" stroke="#f59e0b"/>
                    <path d="M17 16a4 4 0 0 0-8 0c-2.5 0-4 1.5-4 3 0 .5.5 1 1 1.5h13a4 4 0 0 0 0-5" fill="#e2e8f0" stroke={p.inkMuted}/>
                  </svg>
                );
              };
              return (
                <div key={i} style={{
                  ...
                  border: `1px solid ${w.alert ? p.ember + '66' : p.line}`,
                  borderRadius: 10, padding: '10px 6px',
                  textAlign: 'center', position: 'relative',
                }}>
                  {w.alert && (
                    <div style={{ position: 'absolute', top: -6, right: -6, width: 16, height: 16, borderRadius: 8, background: p.ember, color: '#fff', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>!</div>
                  )}
                  <div style={{ fontSize: 10, color: p.inkMuted, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{w.d} · {w.label}</div>
                  <div style={{ display: 'flex', justifyContent: 'center', margin: '4px 0 2px' }}><Icon/></div>
                  <div style={{ fontFamily: T.display, fontSize: 18, fontWeight: 500, color: p.ink, fontVariantNumeric: 'tabular-nums', lineHeight: 1.1 }}>
                    {w.hi}°<span style={{ color: p.inkMuted, fontWeight: 400 }}>/{w.lo}°</span>
                  </div>
                  <div style={{ fontSize: 10, color: p.inkSoft, marginTop: 2 }}>{w.cond}</div>
                  {w.pop && <div style={{ fontSize: 10, color: p.accent, fontWeight: 600, marginTop: 2 }}>{w.pop}% rain</div>}
                </div>
              );
            })}
          </div>
          {/* Saturday alert */}
          <div style={{ background: p.ember + '14', border: `1px solid ${p.ember}55`, borderRadius: 10, padding: '10px 12px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={p.ember} strokeWidth="2" style={{ marginTop: 2, flexShrink: 0 }}><path d="M12 9v4M12 17h.01M10.3 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></svg>
            <div style={{ fontSize: 12, color: p.inkSoft, lineHeight: 1.45 }}>
              <span style={{ color: p.ink, fontWeight: 600 }}>Saturday looks wet</span> — Mr. Avery added a "rain plan" note: extra tarps, dry bag for sleeping bags, and Class B uniform under rain shells. <span style={{ color: p.accent, fontWeight: 600 }}>See packing list →</span>
            </div>
          </div>
        </div>
