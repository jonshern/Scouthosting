...
function eventLines(event, { orgSlug }) {
  // Stable per RFC 5545 — never change this format or every existing event
  // looks new in subscribers' calendars (duplicates the whole calendar).
  const uid = `${event.id}@${orgSlug}.compass.app`;
...
export function icsForOrg(events, { orgSlug, displayName }) {
  const lines = [
    ...
    `X-WR-CALDESC:${escIcs(`${displayName} events on Compass`)}`,
  ];
