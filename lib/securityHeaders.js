...
//     font-src    'self' fonts.gstatic.com data:   ← html2canvas inlines fonts
...
const CSP =
  ...
  // `data:` is needed for html2canvas — the support-widget's screenshot
  // capture inlines page fonts as data: URIs into a cloned DOM so they
  // render in the rasterized image. Cheap to allow; no external attack
  // surface (data: URIs are inlined in the document).
  "font-src 'self' https://fonts.gstatic.com data:; " +
  "img-src 'self' data: blob: https:; " +
  "connect-src 'self' https:; " +
  "form-action 'self'; " +
  "frame-ancestors 'none'; " +
  // 'self' is needed for the GrapesJS canvas, which mounts a same-origin
  // iframe to host the editable preview document.
