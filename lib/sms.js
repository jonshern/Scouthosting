...
async function twilioSend({ to, body }) {
  ...
  if (!sid || !token || !FROM) {
    console.warn("[sms/twilio] TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM missing; falling back to console.");
...
export async function sendSms({ to, body }) {
  ...
  switch (DRIVER) {
    ...
    default:
      console.warn(`[sms] driver "${DRIVER}" not implemented; using console.`);
      return consoleSend({ to: phone, body });
  }
}
