 ...
 * an org admin OR if their linked Member has a position granting one of
 * the required scopes. Falls through to a 403 with a helpful message
 * otherwise.
 *
 * Expects `req.user`, `req.role` (org membership role: "leader"|"admin"|
 * other), and `req.member` (the Member row linked to the user via email)
 * to already be populated by upstream middleware. If `req.member` isn't
 * available the check still works — admins/leaders pass on role alone.
 */
export function requireScope(...wanted) {
  const wantSet = new Set(wanted);
  return function scopeGate(req, res, next) {
    // Org admins always pass — they are the catch-all leader role.
    if (req.role === "admin") return next();
    // The member's position(s) determines fine-grained access.
    const positions = [req.member?.position].filter(Boolean);
    const granted = scopesForPositions(positions);
    for (const w of wantSet) {
      if (granted.has(w)) return next();
    }
    // A coarse "leader" without the specific scope still passes when the
    // wanted scope is COMMITTEE_CHAIR / COMMITTEE / UNIT_LEADER — those
    // overlap heavily with general leader responsibilities. The TREASURER,
    // SECRETARY, and ADVANCEMENT scopes are the genuinely narrow ones.
    const NARROW = new Set([SCOPES.TREASURER, SCOPES.SECRETARY, SCOPES.ADVANCEMENT]);
    const isNarrow = [...wantSet].every((s) => NARROW.has(s));
    if (req.role === "leader" && !isNarrow) return next();
