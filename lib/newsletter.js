...
/**
 * Pull the recent posts + upcoming events that should pre-fill a new
 * newsletter draft. Membership-gated content (private posts) is *not*
 * filtered out here — the composer decides what to include.
 *
 * @param {Object}   args
 ...
 * @param {number}   [args.eventLimit=8]     Cap on events returned.
...
export async function composeNewsletter({
  ...
  const [posts, events, org] = await Promise.all([
  ...
  return {
    ...
    suggestedIntro: defaultIntro(orgName, posts.length, events.length),
  };
}

function defaultIntro(orgName, postCount, eventCount) {
...
export function renderNewsletterHtml({
  ...
  const headerHtml = `
    ...
    <p style="font-size:12px;color:#5a6258;margin:0 0 24px;letter-spacing:0.04em;text-transform:uppercase">${escapeHtml(orgName)}</p>
  `;
    ...
    <ul style="list-style:none;padding:0;margin:0 0 28px">
      ${posts
        ...
        <li style="margin-bottom:18px;padding-bottom:18px;border-bottom:1px solid #e6dcc0">
          <a href="${escapeHtml(safeBaseUrl)}/posts/${escapeHtml(p.id)}" style="color:#0e3320;text-decoration:none;font-weight:600;font-size:16px">${escapeHtml(p.title || "(untitled)")}</a>
    ...
    <ul style="list-style:none;padding:0;margin:0 0 28px">
      ${events
        ...
        <li style="margin-bottom:14px;display:flex;gap:14px;align-items:flex-start">
          <span style="display:inline-block;min-width:64px;font-family:Newsreader,Georgia,serif;font-size:14px;font-style:italic;color:#3a7ab8">${escapeHtml(formatEventDate(e.startsAt))}</span>
          <span style="flex:1">
            <a href="${escapeHtml(safeBaseUrl)}/events/${escapeHtml(e.id)}" style="color:#0e3320;text-decoration:none;font-weight:600">${escapeHtml(e.title)}</a>
  ...
  const footerHtml = `
    <p style="font-size:12px;color:#5a6258;line-height:1.5;margin-top:36px;padding-top:18px;border-top:1px solid #d4c8a8">
      ...
      Hosted with <a href="https://compass.app" style="color:#0e3320;text-decoration:underline">Compass</a> — independent, not affiliated with Scouting America or BSA.
    </p>`;
