...
const KIND_ANNOUNCEMENTS = "announcements";
...
export async function ensureChannel({
  ...
  const data = { orgId, kind, name, patrolName, eventId };
  if (postPolicy) data.postPolicy = postPolicy;
  channel = await prismaClient.channel.create({ data });
  return channel;
}
...
async function desiredUserIds(channel, prismaClient) {
  ...
  if (kind === KIND_TROOP || kind === KIND_EVENT || kind === KIND_ANNOUNCEMENTS) {
    const rows = await prismaClient.orgMembership.findMany({
      where: { orgId },
      select: { userId: true },
    });
