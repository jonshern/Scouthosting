...
// Stable identity: sort the two user IDs so "Alice → Bob" and "Bob →
// Alice" resolve to the same Channel. Using the sorted pair as the
// channel name lets us do a (orgId, kind, name) findFirst as the
// idempotency key without adding a new column.
function dmChannelName(userAId, userBId) {
  return `dm:${[userAId, userBId].sort().join(":")}`;
}

/**
 * Find or create the DM channel between two users in an org. Both
 * users must already belong to the org (caller's responsibility).
 * Returns the Channel row.
 */
export async function findOrCreateDmChannel(orgId, userAId, userBId, { prismaClient }) {
  if (!orgId || !userAId || !userBId || userAId === userBId) {
    throw new Error("findOrCreateDmChannel: needs orgId + two distinct user IDs");
  }
  if (!prismaClient) throw new Error("findOrCreateDmChannel: prismaClient required");
  const name = dmChannelName(userAId, userBId);
  const existing = await prismaClient.channel.findFirst({
    where: { orgId, kind: KIND_DM, name },
  });
  ...
  return prismaClient.channel.create({
    data: {
      ...
      kind: KIND_DM,
      name,
      // members-only post policy means only the two participants post.
      // Adult leaders / admins / unit-leader-scope holders bypass it
      // via canPostToChannel — same as any other channel.
      postPolicy: "members",
      members: {
        create: [
          { userId: userAId, addedAutomatically: false, role: "member" },
          { userId: userBId, addedAutomatically: false, role: "member" },
        ],
      },
    },
  });
...
/* ------------------------------------------------------------------ */
/* Email-reminder cron for unread DMs                                  */
/* ------------------------------------------------------------------ */

const DM_REMINDER_DELAY_MS = 30 * 60 * 1000; // 30 minutes

/**
 * Find DM messages older than the reminder threshold whose recipient
 * hasn't read them and we haven't yet emailed about. Returns an array
 * of { message, recipient } pairs ready for the caller to dispatch
 * email through their mail driver.
 *
 * The actual "send the email" step lives in the cron loop (server/
 * index.js) so this helper stays pure and DB-driven — easier to test
 * and re-use from a one-shot script.
 */
export async function findUnreadDmRemindersDue({ now = new Date(), prismaClient }) {
  if (!prismaClient) throw new Error("findUnreadDmRemindersDue: prismaClient required");
  const cutoff = new Date(now.getTime() - DM_REMINDER_DELAY_MS);

  const candidates = await prismaClient.message.findMany({
    where: {
  ...
  for (const m of candidates) {
    // Identify the (single) recipient — the channel member that isn't
    // the author. DMs always have exactly two members, but we tolerate
    // membership drift defensively.
    const others = m.channel.members.filter((cm) => cm.userId !== m.authorId);
    if (!others.length) continue;
    for (const cm of others) {
      // Already-read filter: the recipient's high-water mark is at or
      // past the message → they've seen it; no reminder needed.
      if (cm.lastReadAt && cm.lastReadAt >= m.createdAt) continue;
      out.push({ message: m, recipientUserId: cm.userId, channel: m.channel });
    }
  }
  return out;
}
...
/**
 * Stamp a Message as having had its email reminder sent. Idempotent —
 * the cron caller updates this after every successful mail send so we
 * don't re-fire on the next sweep.
 */
export async function markDmReminderSent(messageId, { now = new Date(), prismaClient }) {
  if (!prismaClient) throw new Error("markDmReminderSent: prismaClient required");
