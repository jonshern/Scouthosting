...
const FROM_DEFAULT =
  process.env.MAIL_FROM || "Scouthosting <noreply@example.invalid>";
...
function consoleSend(msg) {
  ...
  const id = `console-${Date.now().toString(36)}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  const preview = (msg.text || msg.html || "").slice(0, 200).replace(/\n/g, "\\n");
  const trunc = (msg.text || msg.html || "").length > 200 ? "..." : "";
  console.log(
    `[mail/console] ${id}\n  from:  ${from}\n  to:    ${msg.to}\n  subj:  ${msg.subject}\n  body:  ${preview}${trunc}`
  );
...
async function resendSend(msg) {
  ...
  if (!key) {
    console.warn(`[mail/resend] RESEND_API_KEY missing; falling back to console.`);
...
async function smtpSend(msg) {
  ...
  if (!host) {
    console.warn(`[mail/smtp] SMTP_HOST missing; falling back to console.`);
    return consoleSend(msg);
  }
  if (!smtpTransporter) {
...
export async function send(msg) {
  ...
  switch (DRIVER) {
    ...
    case "ses":
    default:
      console.warn(`[mail] driver "${DRIVER}" not implemented; using console.`);
      return consoleSend(msg);
  }
}
