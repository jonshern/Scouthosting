...
// shape can be tested without touching HTML, and a future mobile/native
// dashboard can reuse the same model.
//
...
// Stable category → accent colour mapping. New categories fall back to
// the primary token rather than picking randomly, so the dashboard
// doesn't shuffle when a leader adds a new category.
const CATEGORY_COLORS = {
  meeting: TOKENS.sky,
  campout: TOKENS.accent,
  service: TOKENS.teal,
  "court-of-honor": TOKENS.raspberry,
  ceremony: TOKENS.raspberry,
  training: TOKENS.butter,
  fundraiser: TOKENS.ember,
  trip: TOKENS.plum,
  highadventure: TOKENS.plum,
  social: TOKENS.butter,
};

export function categoryColor(category) {
  if (!category) return TOKENS.primary;
  const key = String(category).toLowerCase().replace(/\s+/g, "-");
  return CATEGORY_COLORS[key] || TOKENS.primary;
}
...
export async function buildDashboardModel({ prisma, orgId, now = new Date() }) {
  ...
  return {
    greeting,
    stats: {
      scouts: { value: youthCount, hint: `${adultCount} adult leader${adultCount === 1 ? "" : "s"}`, color: TOKENS.sky },
      rsvps: {
