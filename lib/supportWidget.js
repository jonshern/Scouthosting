...
//
// When the user picks the "bug" category, a "Capture screenshot"
// button appears that runs html2canvas (loaded on-demand from a CDN)
// against the current viewport. The PNG is sent as multipart along
// with the rest of the form. /help saves it through the standard
// storage driver and renders it inline on /__super/support/:id.
...
<div id="cmp-support-root" data-surface="${safeSurface}" aria-live="polite">
  ...
  <form id="cmp-support-panel" method="post" action="/help" enctype="multipart/form-data" hidden>
    ...
    <input type="hidden" name="viewportWidth" id="cmp-support-vw" value="">
    <input type="hidden" name="viewportHeight" id="cmp-support-vh" value="">
    <header>
    ...
    <label>
      Category
      <select name="category" id="cmp-support-category">
    ...
    <div id="cmp-support-screenshot" hidden>
      <button type="button" id="cmp-support-capture">📷 Capture screenshot of this page</button>
      <input type="file" name="screenshot" id="cmp-support-shot-input" accept="image/png" hidden>
      <div id="cmp-support-shot-preview" hidden>
        <img id="cmp-support-shot-img" alt="Captured screenshot">
        <button type="button" id="cmp-support-shot-clear" aria-label="Remove screenshot">×</button>
      </div>
    </div>
    <footer>
...
<style>
  ...
  /* The [hidden] attribute's default display:none is overridden by the
     explicit display:flex above; restore the closed state explicitly. */
  #cmp-support-panel[hidden] { display: none; }
  ...
  /* Screenshot section — only shown when category=bug */
  #cmp-support-screenshot { display: flex; flex-direction: column; gap: 6px; }
  #cmp-support-capture {
    background: var(--surface, #fff); color: var(--ink, #0f172a);
    border: 1px dashed var(--line, #e2e8f0); border-radius: 7px;
    padding: 8px 10px; font-size: 12px; cursor: pointer; font-family: inherit;
  }
  #cmp-support-capture:hover { border-color: var(--accent, #1d4ed8); color: var(--accent, #1d4ed8); }
  #cmp-support-capture[disabled] { opacity: 0.6; cursor: progress; }
  #cmp-support-shot-preview {
    position: relative;
    border: 1px solid var(--line, #e2e8f0); border-radius: 7px; padding: 4px;
    background: var(--surface, #fff);
  }
  #cmp-support-shot-img { display: block; width: 100%; height: auto; border-radius: 4px; }
  #cmp-support-shot-clear {
...
(function(){
  ...
  var category    = document.getElementById('cmp-support-category');
  var shotSection = document.getElementById('cmp-support-screenshot');
  var captureBtn  = document.getElementById('cmp-support-capture');
  var shotInput   = document.getElementById('cmp-support-shot-input');
  var shotPreview = document.getElementById('cmp-support-shot-preview');
  var shotImg     = document.getElementById('cmp-support-shot-img');
  var shotClear   = document.getElementById('cmp-support-shot-clear');
  var vwInput     = document.getElementById('cmp-support-vw');
  var vhInput     = document.getElementById('cmp-support-vh');
  ...
  function open(){
    ...
    if (vwInput) vwInput.value = window.innerWidth;
    if (vhInput) vhInput.value = window.innerHeight;
    var f = panel.querySelector('input[name="subject"]');
    f && f.focus();
    syncCategory();
  }
  function shut(){
  ...
  function syncCategory(){
    if (!category || !shotSection) return;
    shotSection.hidden = (category.value !== 'bug');
  }
  ...
  category && category.addEventListener('change', syncCategory);

  // Lazy-load html2canvas only when the user actually clicks Capture.
  // Saves ~50KB on every page render for users who never report a bug.
  ...
  captureBtn && captureBtn.addEventListener('click', function(){
    ...
    // Hide the panel briefly so it doesn't appear in its own screenshot.
    ...
    loadHtml2Canvas()
      ...
      .catch(function(err){
        alert('Couldn\\'t capture the screenshot: ' + (err && err.message || 'unknown error'));
      })
      .then(function(){
  ...
  shotClear && shotClear.addEventListener('click', function(){
    shotInput.value = '';
