...
export const classicTroop = {
  ...
  build(org) {
    ...
    // Block IDs are random per call. Since the canvas array is the
    // single source of truth for ordering, re-applying the template
    // simply overwrites customBlocks wholesale — no sectionOrder
    // references to keep stable across applications.
    ...
    // Block lineup that follows the built-in hero on the homepage:
    // about-text → live events → photos → posts → CTA → contact card.
    const customBlocks = [
      {
        id: aboutBlockId,
        type: "text",
