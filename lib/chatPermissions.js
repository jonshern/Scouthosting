 ...
 * @param {object} actor — { role, isLeader, scopes, channelMembership, member }
 *   role: org membership role ("admin" | "leader" | "scout" | "parent" | etc.)
 *   isLeader: convenience for role ∈ {admin, leader}
 *   scopes: optional Set of position scopes (lib/permissions.SCOPES). When
 *           it includes "unit-leader" the actor is treated as Cubmaster /
 *           Scoutmaster / Skipper regardless of their OrgMembership role.
 *   channelMembership: ChannelMember row for this user, or null. The
 *           `.role` field on this row decides channel-level ownership
 *           ("owner" | "member"); "owner" lets the user post into a
 *           channel set to the announce-only "leaders" policy, like a
 *           den leader posting in their den's announcements channel.
...
export function canPostToChannel(channel, actor) {
  ...
  const hasUnitLeaderScope = actor.scopes?.has?.("unit-leader") === true;
  const isLeader = isAdmin || actor.role === "leader" || actor.isLeader === true || hasUnitLeaderScope;

  // Workspace-level "post anywhere" tier: admins, the legacy leader role,
  // and unit-leader-scope holders (Cubmaster, Scoutmaster, Skipper, …).
  if (isLeader) return { ok: true };

  // Channel-level owner: a den leader / patrol leader who's been
  // designated as the manager of this specific channel. Pass-through
  // only matters on announce-style "leaders" channels — regular
  // channels are membership-driven and owner gives no posting boost.
  const isChannelOwner = actor.channelMembership?.role === "owner";

  const policy = channel.postPolicy || "members";

  if (policy === "leaders") {
    if (isChannelOwner) return { ok: true };
