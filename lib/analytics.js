 ...
 *   }
 */
export async function summarize({ orgId, since, until } = {}, prisma = defaultPrisma) {
  const where = analyticsWhere({ orgId, since, until });
  const rows = await prisma.auditLog.groupBy({
    by: ["action"],
    where,
    _count: { _all: true },
...
export async function topPaths(
  { orgId, surface = null, since, until, limit = 10 } = {},
  prisma = defaultPrisma,
) {
  const where = analyticsWhere({ orgId, since, until, action: "analytics:page-view" });
...
export async function topClicks(
  ...
  const where = analyticsWhere({ orgId, since, until, action: "analytics:element-clicked" });
 ...
 * breaking right now".
 */
export async function recentErrors({ orgId, since, limit = 20 } = {}, prisma = defaultPrisma) {
  const where = analyticsWhere({ orgId, since, action: "analytics:client-error" });
  const rows = await prisma.auditLog.findMany({
    where,
 ...
 */
export async function recentFetchFails({ orgId, since, limit = 20 } = {}, prisma = defaultPrisma) {
  const where = analyticsWhere({ orgId, since, action: "analytics:fetch-failed" });
  const rows = await prisma.auditLog.findMany({
    where,
    orderBy: { createdAt: "desc" },
    take: limit,
 ...
 */
export async function pageViewsByDay({ orgId, since, until } = {}, prisma = defaultPrisma) {
  const where = analyticsWhere({ orgId, since, until, action: "analytics:page-view" });
  const rows = await prisma.auditLog.findMany({
    where,
...
/**
 * Marketing-funnel rollup: how many people land on the marketing site,
 * how many click a `data-track` CTA, how many of those reach the
 * /signup page, and how many actually sign up. Each step's conversion
 * rate is computed against the previous step.
 *
 * Returns:
 *   {
 *     stages: [
 *       { key, label, count, conversion: number|null }, ...
 *     ],
 *     overall: number|null,   // signups / marketing visits
 *   }
 */
export async function marketingFunnel({ since, until } = {}, prisma = defaultPrisma) {
  // Stage 1: marketing-surface page views.
  const pvWhere = analyticsWhere({ since, until, action: "analytics:page-view" });
  const pageViews = await prisma.auditLog.findMany({
    where: pvWhere,
    select: { summary: true },
  });
  ...
  for (const row of pageViews) {
    const dims = parseDims(row.summary);
    if (!dims) continue;
    if (dims.surface === "marketing") {
      marketingViews += 1;
      if (dims.path && /\/signup(?:\.html)?(?:[?#]|$)/.test(dims.path)) signupViews += 1;
    }
  }

  // Stage 2: marketing-surface CTA clicks (data-track elements). Any
  // labelled click on the marketing surface counts — we don't filter
  // by label because every marketing CTA points toward signup.
  const clickWhere = analyticsWhere({ since, until, action: "analytics:element-clicked" });
  const clicks = await prisma.auditLog.findMany({
    where: clickWhere,
    select: { summary: true },
  });
  ...
  for (const row of clicks) {
    const dims = parseDims(row.summary);
    if (!dims) continue;
    if (dims.surface === "marketing" && dims.label) marketingClicks += 1;
  }

  // Stage 3: signups (server-side event recorded by /signup handler).
  const signupWhere = analyticsWhere({ since, until, action: "analytics:user-signed-up" });
  const signups = await prisma.auditLog.count({ where: signupWhere });

  const stage = (key, label, count, prevCount) => ({
    key,
    label,
    count,
    conversion: prevCount > 0 ? count / prevCount : null,
  });
  const stages = [
    stage("views", "Marketing page views", marketingViews, 0),
    stage("clicks", "CTA clicks", marketingClicks, marketingViews),
    stage("signup_view", "/signup page view", signupViews, marketingClicks),
    stage("signups", "Account signups", signups, signupViews),
  ];
  // Drop the trailing "conversion" on the first stage; it's not meaningful.
  stages[0].conversion = null;
  return {
    stages,
    overall: marketingViews > 0 ? signups / marketingViews : null,
  };
}

function analyticsWhere({ orgId, since, until, action } = {}) {
  const out = {};
