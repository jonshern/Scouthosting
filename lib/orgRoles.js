...
/**
 * Auto-seed the canonical Subgroup rows for a freshly-provisioned org.
 * For Cub Scout Packs and Girl Scout Troops we create one Subgroup per
 * preset (Lion → "Lion Den", etc.); for unit types without a fixed
 * vocabulary we no-op and leave subgroup setup to the leader.
 *
 * Idempotent — uses skipDuplicates so calling twice (or running on an
 * existing org) won't blow up. Returns the array of created records.
 */
export async function seedSubgroupsForOrg({ prisma, org }) {
  const presets = subgroupPresets(org.unitType);
  if (!presets.length) return [];

  const vocab = subgroupVocab(org.unitType);
  ...
  await prisma.subgroup.createMany({ data: rows, skipDuplicates: true });
  return prisma.subgroup.findMany({
    where: { orgId: org.id, name: { in: rows.map((r) => r.name) } },
  });
}
