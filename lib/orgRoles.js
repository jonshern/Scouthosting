...
export function buildSeedBroadcastChannels(unitType) {
  ...
  // Pack and Girl Scout Troop youth are too young for direct contact —
  // their contact info IS their parents'. Broadcasts to a den/level
  // therefore resolve to the *parents* of the youth in that section,
  // not the youth themselves. The "parents-of-youth" audience mode
  // matches the youth via the rules and then walks Member.parentIds.
  return presets.map((p) => ({
    name: `${p.label} ${singularCap}`,
    description: `Parents of ${p.label} (${p.grade}) ${vocab.singular} youth`,
    isYouth: true,
    patrols: [p.label],
