...
// structured JSON to stdout AND persists to the ErrorLog table for
// the in-app /__super/errors viewer. Any log-aggregation backend
// can also pick the stdout up:
//
...
/**
 * Stable fingerprint for grouping. Hash of (error name + first stack
 * frame) — same exception thrown from the same line gets the same
 * fingerprint regardless of when or who triggered it. Falls back to
 * the message when the stack is unavailable.
 */
export function fingerprintError(err) {
  const e = err instanceof Error ? err : new Error(String(err));
  const firstFrame = (e.stack || "").split("\n").find((l) => l.trim().startsWith("at "))?.trim() || "";
  const seed = `${e.name || "Error"}|${firstFrame || e.message || "unknown"}`;
...
/**
 * Best-effort DB persistence. Swallows write failures (a DB outage
 * shouldn't worsen an error). Returns nothing — fire-and-forget.
 *
 * orgId is included only when the resolved org actually exists in
 * the request — apex/marketing throws have req.org = null.
 *
 * Truncates oversized fields to keep a runaway error (e.g. a giant
 * stack frame from a transitive lib) from blowing past Postgres
 * column limits.
 */
async function persistError(err, req, { service }) {
  try {
    const e = err instanceof Error ? err : new Error(String(err));
    ...
    await prisma.errorLog.create({
      data: {
        ...
        message: truncate(e.message || "(no message)", 2000),
        stack: truncate(e.stack || null, 8000),
        errorName: truncate(e.name || null, 200),
        method: req?.method || null,
        path: truncate(req?.path || null, 500),
    ...
    // The DB persist itself failed. Log it via the structured logger
    // (still goes to stdout so an external aggregator catches the
    // outage). Don't re-throw — the original error has already been
    // logged; the user-facing 500 has already been sent.
    log.warn("errorLog persist failed", { err: writeErr });
  }
}
...
export function expressErrorHandler({ service } = {}) {
  ...
  return function compassErrorHandler(err, req, res, next) {
    ...
    // Fire-and-forget DB persist so /__super/errors has something to
    // show. Doesn't block the response.
    persistError(err, req, { service });
...
export function installFatalHandlers({ service } = {}) {
  ...
  process.on("unhandledRejection", (reason) => {
    ...
    // We don't exit on rejections, so there's time to persist. Fatal
    // uncaughtException above intentionally skips DB writes — exit
    // window is 100ms and a Postgres round-trip won't complete.
    persistError(err, null, { service });
  });
}
