...
- [x] **Photo attachments.** Drop a photo from the chat composer (web file picker for v1; mobile picker is the next mobile-app cycle). Schema: `Photo.albumId` is now optional, `Photo.messageId` is the new chat-tie, `uploaderUserId` tracks the sender. Mutually exclusive — a Photo is either gallery-context or chat-context. New `POST /api/v1/channels/:id/photos` (multer single-file, 8 MB cap, jpeg/png/webp/heic/gif allowlist) creates an orphan Photo row + moves the file via the existing `lib/storage.js`; the client then sends a normal `/messages` POST with `attachment={kind:"photo", photoId, caption}`. The send-handler links the Photo to the message and refuses (404 / 403 / 409) if the photo doesn't belong to the caller / is already linked / is in another org. The existing `/uploads/:filename` route gained a chat-photo branch that authorizes via the message's channel membership instead of album visibility; deleted-message photos serve a tombstone. `enrichPhotoAttachments()` patches the message DTO with `url` + `mimeType` + dimensions in a single batched query per page. 6 new tests (179 server total).
